...
from UserServer import settings
from commons.core.auth import index_login_required
from commons.core.security import create_access_token, get_user_by_token
        ...
        if password is not None:
            ...
            if userType == "student":
                try:
                    ...
                    token = create_access_token({"userType": userType, "user_id": user.id, "person_id": person.id,"currentAuthority":"user"})
                    person.token = token
                    user.save()
                    person.save()
                ...
                try:
                    ...
                    token = create_access_token({"userType": userType, "user_id": user.id, "admin_id": admin.id, "currentAuthority": "admin"})
                    admin.token = token
                    user.save()
                    admin.save()
                    user = authenticate(request, username=email, password=password)
...
def user_login(request):
    ...
    if user is not None:
        ...
        if userType == "student":
            ...
            token = create_access_token({"userType": userType, "user_id": user.id, "person_id": student.id, "currentAuthority": "user"})
            student.token = token
            student.save()
        if userType == "teacher":
            teacher = Admin.objects.get(user__username=email)
            token = create_access_token({"userType": userType, "user_id": user.id, "admin_id": teacher.id, "currentAuthority": "admin"})
            teacher.token = token
            teacher.save()
            currentAuthority = "admin"
        # login(request, user)
...
def current_user(request):
    ...
    currentAuthority = user_info.get("currentAuthority")
    ...
    if user_info:
        try:
            current_user = User.objects.get(id=user_info["user_id"])
            if currentAuthority == "user":
                student = Person.objects.get(id=user_info["person_id"])
                ...
                teacher = Admin.objects.get(id=user_info['admin_id'])
                user_content = {
...
def edit_usericon(request):
    user_info = get_user_by_token(request)
    person = Person.objects.get(id=user_info["person_id"])
    form = UploadImageForm(request.POST, request.FILES)
    if form.is_valid():
        person.user_icon = request.FILES['image']
        person.save()
...
def edit_userInfo(request):
    ...
    modified = False
    person = Person.objects.get(pk=user_info.get("person_id"))
    if "tags" in request_content.keys():
        person.tags = request_content.get("tags")
        msg["tags"] = 1
        msg["error_code"] = 200
        response_msg += "tags "
        modified = True
    if "phone" in request_content.keys():
        person.phone = request_content.get("phone")
        msg["phone"] = 1
        msg["error_code"] = 200
