...
class Topic(models.Model):
    topic_title = models.CharField(
        max_length=20, verbose_name="Topic title"
    )  # Topic title
    ...
    topic_content = models.TextField(
        null=True
    )  # introduction to the main content of the topic
    views = models.PositiveIntegerField("浏览量", default=0)  # view numbers of the topic
    create_time = models.DateTimeField(auto_now=True)
    topic_img = models.ImageField(
        null=True, blank=True, verbose_name="topic_image", upload_to="topic_imgs/"
    )
...
class MyCourse(models.Model):
    ...
    content = RichTextUploadingField("内容", default="请输入课程内容")
    ...
    update_date = models.DateTimeField(
        max_length=20, verbose_name="更新时间", auto_now=True
    )
